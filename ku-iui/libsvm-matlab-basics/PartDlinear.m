...
for i = 1:Nlog2c
    for j = 1:Nlog2g
        ...
        params = ['-q -t 0 -v 5 -c ', num2str(2^log2c), ' -g ', num2str(2^log2g)];
...
ylabel('Log_2c');
title('Grid Search over c and \gamma for linear kernel');
saveas(hm, 'img/heatLinear.jpg');

% Train and test SVM
% Train model using specified kernel and best cost & gamma values
params = ['-t 0 -v 5 -c ', num2str(bestC), ' -g ', num2str(bestG)];
accuracy = svmtrain(labels, features, params);
disp('Linear: done!');
